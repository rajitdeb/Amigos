...
class _FollowRequestsState extends State<FollowRequests> {
  List<AmigosFollowRequestUser> amigosFollowRequestUserList =
  ...
  Widget build(BuildContext context) {
    return isDataLoading == true
        ...
        : Container(
            ...
            child: amigosFollowRequestUserList.isEmpty
                ? Column(
  ...
  Future<void> _deleteFollowRequestFromUser(int index) async {
    ...
    await FirebaseFirestore.instance
        .collection('followRequests')
        .doc(FirebaseAuth.instance.currentUser!.uid)
        .update({'followRequestList': userDetails}).then((value) {
      setState(() {
        isDataLoading = true;
  ...
  void _addUserToMyFollowerList(int index) async {
    ...
    AmigosFollowRequestUser followRequestUser =
        amigosFollowRequestUserList[index];
      ...
      if (snapshot.exists) {
        ...
        if (followerList.followers.isEmpty) {
          ...
          await FirebaseFirestore.instance
              .collection('users')
              .doc(FirebaseAuth.instance.currentUser!.uid)
              .update({'followers': followerList.followers});
        }
  ...
  void _addCurrentUserToFollowingList(
      AmigosFollowRequestUser followRequestUser, int index) async {
    /*
    ...
    GetUserBloc()
      ...
      FollowingDetails followingDetails = FollowingDetails(amigosUser.userId,
          amigosUser.fullName, amigosUser.username, amigosUser.profileImg);
        ...
        if (snapshot.exists) {
          ...
          if (followingList.following.isEmpty) {
            ...
            await FirebaseFirestore.instance
                .collection('users')
                .doc(followRequestUser.userId)
                .update({'following': followingList.following});
          }
